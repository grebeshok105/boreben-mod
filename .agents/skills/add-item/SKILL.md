...
# Add Item — Boreben Addon

Базовый pipeline для нового предмета в аддоне. **Все ID — под неймспейсом `boreben`**, не `superheroes`.

1. **Класс предмета** в `src/main/java/com/boreben/mod/item/<Name>Item.java`
   - extends `Item` (или специальная база если нужно)

2. **Регистрация в `BorebenItems`**
   ```java
   public static final Item FOO = register("foo", new FooItem(new Item.Properties()...));

   private static Item register(String name, Item item) {
       return Registry.register(BuiltInRegistries.ITEM,
           ResourceLocation.fromNamespaceAndPath("boreben", name), item);
   }
   ```

3. **Текстура**: `assets/boreben/textures/item/<name>.png` (16x16 обычно)

4. **Модель**: `assets/boreben/models/item/<name>.json`
   ```json
   {"parent": "minecraft:item/generated", "textures": {"layer0": "boreben:item/<name>"}}
   ```

5. **Локализация**: `assets/boreben/lang/en_us.json` + `lang/ru_ru.json`
   ```json
   "item.boreben.<name>": "Foo"
   ```

6. **Recipe** (опционально): `data/boreben/recipes/<name>.json`

7. **Добавить в creative-вкладку основного мода** через `BorebenItemGroupHook`:
   ```java
   ItemGroupEvents.modifyEntriesEvent(SUPERHEROES_TAB).register(entries -> {
       entries.accept(BorebenItems.FOO);
   });
   ```
...
## Чек-лист
- [ ] ID в неймспейсе `boreben`, не `superheroes`
- [ ] Текстура и модель под `assets/boreben/...`
